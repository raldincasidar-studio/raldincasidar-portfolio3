    ...
    <div v-else class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-3xl bg-white shadow-sm">
          ...
          <div class="space-y-6 p-5 sm:p-8">
            ...
            <div v-if="isWorks" class="space-y-5 border-t pt-6">
              ...
              <div class="grid gap-4 sm:grid-cols-2"><label class="admin-label">Project type<input v-model="draft.caseStudy.type" class="admin-input mt-2" placeholder="MOBILE APP" /></label></div>
        ...
        <template v-if="isWorks">
          <div class="admin-editor-card"><div class="admin-editor-heading"><div><p class="admin-eyebrow">Case study page · contribution</p><h2 class="admin-section-title">Role and scope</h2></div></div><div class="grid gap-4 sm:grid-cols-2"><label class="admin-label">Role<input v-model="draft.caseStudy.contribution.role" class="admin-input mt-2" /></label><label class="admin-label">Discipline<input v-model="draft.caseStudy.contribution.discipline" class="admin-input mt-2" /></label></div><div class="mt-4"><label class="admin-label">Scope</label><div class="mt-2 flex gap-2"><input v-model="draft._scopeInput" class="admin-input" placeholder="Frontend, Backend, UX…" @keyup.enter="addScope" /><button class="admin-button-secondary" @click="addScope">Add</button></div><div class="mt-3 flex flex-wrap gap-2"><span v-for="(scope, index) in draft.caseStudy.contribution.scope" :key="scope" class="rounded-full bg-[#BBE6F6] px-3 py-1 text-sm text-[#132132]">{{ scope }} <button class="ml-1" @click="removeScope(index)">×</button></span></div></div></div>
