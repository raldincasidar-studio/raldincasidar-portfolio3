...
export function loadAdmin(req, res, next) {
  connectDatabase().then(async () => {
    ...
    if (!admin?.active) return res.status(403).json({ error: { code: 'FORBIDDEN', message: 'Admin account is inactive' } })
    if ((admin.sessionVersion || 1) !== (req.auth.sessionVersion || 1)) return res.status(401).json({ error: { code: 'SESSION_REVOKED', message: 'Session expired, please sign in again' } })
    req.admin = admin
    next()
  }).catch(next)
}
